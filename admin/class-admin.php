...
class IRP_Admin {
    ...
    public function render_leads(): void {
        ...
        if (isset($_GET['lead'])) {
            ...
            $propstack_columns_exist = in_array('propstack_id', $columns);

            // Handle DB repair action
            if (isset($_GET['irp_fix_db']) && $_GET['irp_fix_db'] === '1' && current_user_can('manage_options')) {
                // Add missing Propstack columns
                if (!in_array('propstack_id', $columns)) {
                    $wpdb->query("ALTER TABLE {$table} ADD COLUMN propstack_id bigint(20) unsigned DEFAULT NULL AFTER email_sent_at");
                }
                if (!in_array('propstack_synced', $columns)) {
                    $wpdb->query("ALTER TABLE {$table} ADD COLUMN propstack_synced tinyint(1) NOT NULL DEFAULT 0 AFTER propstack_id");
                }
                if (!in_array('propstack_error', $columns)) {
                    $wpdb->query("ALTER TABLE {$table} ADD COLUMN propstack_error text DEFAULT NULL AFTER propstack_synced");
                }
                if (!in_array('propstack_synced_at', $columns)) {
                    $wpdb->query("ALTER TABLE {$table} ADD COLUMN propstack_synced_at datetime DEFAULT NULL AFTER propstack_error");
                }

                echo '<div class="notice notice-success"><p><strong>✅ Datenbank repariert!</strong> Propstack-Spalten wurden hinzugefügt. Bitte Seite neu laden.</p></div>';
            ...
            // Only query propstack data if columns exist
            $debug_data = null;
            if ($propstack_columns_exist) {
            ...
            echo "---\n";
            echo 'propstack_id Spalte existiert: ' . ($propstack_columns_exist ? 'JA ✅' : 'NEIN ❌') . "\n";
            echo "---\n";
            if ($propstack_columns_exist) {
                echo 'DB Result Lead ID: ' . esc_html($debug_data->id ?? 'NULL') . "\n";
                echo 'propstack_id: ' . esc_html($debug_data->propstack_id ?? 'NULL') . "\n";
            ...
            // Show repair button if columns are missing
            if (!$propstack_columns_exist) {
                $fix_url = add_query_arg('irp_fix_db', '1');
                echo '<p style="margin-top: 10px;"><a href="' . esc_url($fix_url) . '" class="button button-primary" onclick="return confirm(\'Datenbank-Spalten hinzufügen?\');">🔧 Datenbank reparieren (Propstack-Spalten hinzufügen)</a></p>';
            }
